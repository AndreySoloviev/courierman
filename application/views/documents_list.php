    	...
    	<h2 style="padding: 10px 0px;">Заявки курьеру</h2>        
		  	...
		  	foreach ($documents as $document) { 
			    ...
			    if ( $date != $document->date_to_go )
			    {
			    	...
			    	echo "<h3 style='padding-bottom: 6px;'>".$this->useful->date_human_from_mysql($date).", ".$this->useful->day_of_week_from_mysql($date);
			    	if ($date == date("Y-m-d")) 
			    		echo "<span style='color: #007a05;'> (сегодня)</span>";
			    	echo  "</h3>";
